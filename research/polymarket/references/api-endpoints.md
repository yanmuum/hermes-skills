...
GET /trades?user=0xADDRESS&limit=N     ← Correct way to filter by user
```

**IMPORTANT: `/trades?maker=0x...` does NOT filter by user.** Use `?user=0x...` instead.

Trade fields: `side` (BUY/SELL), `size`, `price`, `timestamp`,
`title`, `slug`, `outcome`, `proxyWallet`, `transactionHash`, `conditionId`.

**Known limitations:**
- `?user=` returns 500 trades max, approximately the last month of data only
- Only BUY records are returned (opening positions, not sells/closes)
- No working pagination observed
- `proxyWallet` field shows the user's proxy address (often same as main wallet)

### User Positions (Current Open Only)

```
GET /positions?user=0xADDRESS&limit=2000
```

Returns current open positions only (does NOT return historical closed positions).
The `closed=true` parameter does not appear to work.

Position fields: `proxyWallet`, `asset`, `conditionId`, `size`, `avgPrice`,
`initialValue`, `currentValue`, `cashPnl`, `percentPnl`, `totalBought`,
`realizedPnl`, `percentRealizedPnl`, `curPrice`, `redeemable`, `mergeable`,
`title`, `slug`, `outcome`, `oppositeOutcome`, `endDate`, `negativeRisk`.

Useful for calculating PnL on settled positions:
- `cashPnl` = net profit/loss in USDC
- `percentPnl` = percentage return
- If `curPrice` is 1.00 and `redeemable` is true, position is settled as a win
- If `curPrice` is 0.00, position is settled as a loss
