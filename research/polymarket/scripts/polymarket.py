...
Usage:
    ...
    python3 polymarket.py trades [--limit 10] [--market CONDITION_ID] [--user ADDRESS]
    python3 polymarket.py user-trades <ADDRESS> [--limit 500]
    python3 polymarket.py positions <ADDRESS>
"""
...
def cmd_trades(limit: int = 10, market: str = None, user: str = None):
    """Get recent trades, optionally filtered by market or user."""
...
def cmd_user_positions(user: str):
    """Get current open positions for a user."""
    positions = _get(f"{DATA}/positions?user={user}&limit=200")
    if not isinstance(positions, list):
        print(f"Unexpected response: {positions}")
        return
    if not positions:
        print(f"No current positions for {user[:20]}...")
        return
    print(f"Current positions for {user[:20]}... ({len(positions)}):\n")
    for p in positions:
        title = p.get("title", "?")
        ...
        print(f"    Entry: {avg_price:.3f}  |  Current: {cur_price:.3f}")
        print(f"    Size: {size:>10.2f} shares  |  PnL: ${cash_pnl:>+8.2f} ({pct_pnl:>+6.2f}%)")
        print()
...
def main():
    ...
    if cmd == "search" and len(args) >= 2:
        ...
        cmd_trades(limit, market, user)
    elif cmd == "user-trades" and len(args) >= 2:
        limit = 500
        if "--limit" in args:
            idx = args.index("--limit")
            limit = int(args[idx + 1]) if idx + 1 < len(args) else 500
        cmd_trades(limit, user=args[1])
    elif cmd == "positions" and len(args) >= 2:
        cmd_user_positions(args[1])
    else:
