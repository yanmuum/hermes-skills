...
## HTML as Image Output (Fallback)

When the `image_generate` tool is not available (no FAL_KEY configured, no image gen API), you can produce a visual output by creating an HTML/CSS artifact and screenshotting it with Playwright. This is useful for promotional posters, banners, teaser cards, and other fixed-dimension visual assets.

When to use this fallback:

- User asks for a visual poster/banner/image but image gen is unavailable
- Output format must be PNG, not openable HTML
- The design is a fixed-format visual (poster, card, banner) — not an interactive page

Workflow:

1. **Design the HTML**. Create a single self-contained HTML file with:
   - Fixed pixel dimensions matching the poster size (e.g., 800x1100px for portrait, 1200x630px for social banner)
   - Inline `<style>` — no external CSS
   - **System fonts only** — avoid Google Fonts CDN imports. Playwright has a known issue where remote font loading can hang the screenshot. Use a fallback stack like: `font-family: -apple-system, 'PingFang SC', 'Microsoft YaHei', sans-serif;`
   - No interactivity needed — this is a static visual
   - Emoji/symbols for icons (🔄 🪙 🐕) instead of image assets
   - All text embedded in HTML (no images of text)

2. **Verify the HTML works** before screenshotting:
   ```
   write_file("/tmp/poster.html", "...")
   ```

3. **Start a local HTTP server** (required by Playwright for local files):
   ```
   terminal(background=True, command="cd /tmp && python3 -m http.server 8899")
   ```

4. **Screenshot with Playwright**. Hermes bundles Playwright at `~/.hermes/hermes-agent/node_modules/.bin/playwright`:
   ```
   terminal(command="""
     cd ~/.hermes/hermes-agent && \
     unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY && \
     npx playwright screenshot --full-page --timeout=15000 \
       http://127.0.0.1:8899/poster.html /tmp/poster.png
   """, timeout=30)
   ```
   - `unset http_proxy` is critical in WSL environments where the Windows proxy breaks localhost
   - Always use `127.0.0.1` not `localhost` for WSL
   - `--full-page` captures the entire layout
   - Set `--timeout` high enough for font rendering

5. **Verify the screenshot** and kill the HTTP server:
   ```
   ls -la /tmp/poster.png   # should be > 50KB for a real poster
   process(action="kill", session_id="proc_...")
   ```

6. **Deliver**. Provide the HTML file path and the PNG path. Offer the user both formats — HTML for editing, PNG for publishing.

Poster design guidelines for this fallback:

- Use CSS gradients, box-shadows, and borders to create depth without images
- Dark crypto-style posters work well: `linear-gradient(135deg, #0b0b2b, #1a0a2e)` with neon accent colors
- Add visual interest with CSS-only effects: floating dots (`radial-gradient` + box-shadow), grid backgrounds, gradient overlays
- Structure the poster: badge/tagline at top, brand area, key stats, feature cards, CTA, footer
- Keep DYOR/disclaimers at the bottom if promoting crypto/finance products

Pitfalls:

- Google Fonts / remote font CDN URLs cause Playwright screenshots to hang with "waiting for fonts to load...". Always use system font stacks.
- In WSL, `localhost` routes through the Windows proxy. Use `127.0.0.1` instead.
- The HTTP server must be running before Playwright connects — start it first, then screenshot.
- Full-page screenshot does not capture overflow hidden elements — ensure the poster content fits within the declared dimensions.
- Playwright's full-page screenshot captures the entire document, so set the poster `<body>` height explicitly.
...
- **Remote Google Fonts block Playwright screenshots.** When using Playwright to snapshot HTML artifacts, Google Fonts CDN links (`fonts.googleapis.com`) cause the screenshot to hang on "waiting for fonts to load..." until timeout. Always use system font stacks — especially for fixed-format poster/banner HTML that will be screenshot as output. Add the hang symptom to your debugging triage when a Playwright screenshot doesn't return.
