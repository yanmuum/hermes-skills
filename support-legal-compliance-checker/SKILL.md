...
## 📋 Trigger Conditions — When This Skill Fires

This skill loads automatically in the "中庭设计项目协作群" group chat when the user sends messages related to:
- 合同 / 协议 / 条款 / 保密 / 合规 / 法律风险 / 法务 / 争议 / 仲裁 / 诉讼
- NDA / non-disclosure / confidentiality agreement
- Intellectual property / 知识产权 / 著作权 / 专利权

When reviewing an NDA or contract for a **natural-person signatory（自然人合作方）**, follow the structured framework in `references/nda-contract-review-framework.md`:
- Use 🔴/🟡/🟢 risk indicator system
- Analyze: IP ownership, liquidated damages, confidentiality scope, dispute venue, duration
- Output a summary table of must-negotiate vs. suggested changes vs. acceptable terms

For generating professional .docx reports, see `references/docx-generation-notes.md`.

### Document Input & Output

**Getting the document:**
- If user sends a Feishu Doc link (`my.feishu.cn/docx/TOKEN`), read via API:
  ```
  GET https://open.feishu.cn/open-apis/docx/v1/documents/{doc_token}/raw_content
  ```
  Requires `docx:document:readonly` permission scoped and app re-published.
- If user sends a Feishu File link (`my.feishu.cn/file/TOKEN` → PDF/Word), they must convert it to a Feishu Doc first (右上方「更多」→「转存为文档」), since the bot likely lacks `drive:file:readonly` permission.
- Fallback: Ask user to paste text directly.

**Output format — user preference:**
- For quick chat review: use the standard 🔴/🟡/🟢 markdown format in chat
- **For downloadable professional report**: generate a `.docx` file using `python-docx`:
  - Use formal report structure: header → basic info → executive summary → detailed clause review → negotiation strategy → disclaimer
  - Save to `/tmp/法律审查意见书_<协议名>.docx`
  - Send via Feishu using the `MEDIA:/path/to/file` protocol in the response
  - Include proper formatting: tables, color-coded risk tags, section headers, page margins
